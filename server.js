...
async function api(req,res,url){
  ...
  if(method==='POST'&&url.pathname==='/api/messages'){const user=requireUser(req,res,db);if(!user)return;const input=await body(req),text=String(input.text||'').trim().slice(0,500);if(!text)return json(res,400,{error:'Mensagem vazia.'});const message={id:crypto.randomUUID(),userId:user.id,userName:user.name,text,createdAt:new Date().toISOString()};db.messages.push(message);writeDb(db);const lower=text.toLowerCase();const reply=lower.includes('hor')?'O treino começa às 19:30. Chega uns 10 min antes!':lower.includes('vaga')||lower.includes('entrar')?'Tem vaga sim! Clica em Entrar na partida da Beira-Mar 🏀':lower.includes('quadra')||lower.includes('onde')?'Vamos jogar na Quadra da Beira-Mar. O mapa mostra a rota.':lower.includes('treino')?'Bora! Treino em grupo vale pontos extras no ranking semestral.':'Fechado! Vou confirmar com o restante do grupo 🙌';setTimeout(()=>{try{const fresh=readDb();fresh.messages.push({id:crypto.randomUUID(),userId:'bot-rafael',userName:'Rafael (bot)',text:reply,createdAt:new Date().toISOString()});writeDb(fresh)}catch(error){console.error('Bot:',error.message)}},900);return json(res,201,{message,botPending:true})}
